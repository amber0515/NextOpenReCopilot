"""
Qt Widgets - 可复用的 PyQt5 UI 组件

此模块包含所有可复用的 UI 组件，不依赖 IDA API。
这些组件可以在 IDA 环境外进行测试。

组件列表:
- EditablePredictionWidget: 可编辑的文本输入组件
- NameTypeWidget: 名称-类型-大小组件
- StructFieldWidget: 结构体字段组件
- EnumFieldWidget: 枚举成员组件
- ComplexTypeWidget: 复杂类型容器
- VariableSelectionWidget: 变量列表选择组件
- OutputWindowButton: 取消按钮组件
"""

from PyQt5 import QtWidgets, QtCore, QtGui
from PyQt5.QtWidgets import (
    QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QWidget, QLineEdit,
    QGroupBox, QGridLayout, QMessageBox, QSpinBox, QCheckBox, QComboBox, QScrollArea
)
...
class NameTypeWidget(QWidget):
    ...
    用于输入名称、类型（或值）和可选的大小字段。
    支持枚举模式（带大小字段和添加/删除按钮）。
    """
...
class StructFieldWidget(NameTypeWidget):
    ...
    继承自 NameTypeWidget，专门用于结构体字段输入。
    包含类型、名称、大小字段以及添加/删除按钮。
    """
    field_added = QtCore.pyqtSignal()
    field_removed = QtCore.pyqtSignal(QWidget)

    def __init__(self, type_str="", name="", size="", parent=None):
        super(StructFieldWidget, self).__init__("Struct Field", name, type_str, size, is_enum=False)
        self.accepted = True
...
class EnumFieldWidget(NameTypeWidget):
    ...
    继承自 NameTypeWidget，专门用于枚举成员输入。
    包含名称、值、大小字段以及添加/删除按钮。
    """
    field_added = QtCore.pyqtSignal()
    field_removed = QtCore.pyqtSignal(QWidget)

    def __init__(self, name="", value="", size="", parent=None):
        super(EnumFieldWidget, self).__init__("Enum Field", name, value, size, is_enum=True)
        self.accepted = True
...
class ComplexTypeWidget(QWidget):
    ...
    def __init__(self, title, type_info=None, parent=None):
        ...
        if self.type_info[2] == "struct":
            ...
            # BUG FIX: Use list() for Python 3 compatibility
