"""
Presentation Layer - Qt Views and Forms

This module contains IDA-integrated view and form classes for the ReCopilot plugin.
Extracted from recopilot_qt.py for better organization and separation of concerns.
"""
...
# Import widgets from qt_widgets
from .qt_widgets import (
    EditablePredictionWidget, ComplexTypeWidget, VariableSelectionWidget
)


class DecompilationViewPluginForm(ida_kernwin.PluginForm):
    """
    A view form for displaying syntax-highlighted decompiled code.

    Uses Pygments for C++ syntax highlighting and integrates with IDA's
    tab system as a persistent, restorable widget.
    """
...
class UserConfirmForm(ida_kernwin.PluginForm):
    """
    Multi-section confirmation dialog for function analysis results.

    Displays and allows editing of:
    - Function name
    - Return type
    - Function arguments (with complex type support)
    - Local variables (with complex type support)
    - Function documentation (brief, details, return, category, algorithm)
    - Inline comments (keyed by line number)

    User can selectively accept/reject each field before applying.
    """
...
class UserConfirmFormForFuncName(ida_kernwin.PluginForm):
    """
    Simplified confirmation dialog specifically for function name recovery.

    Shows original vs predicted function name with a single acceptance checkbox.
    """
    def __init__(self, ea, task_tag, prompt, response_raw, response):
...
class VariableSelectionForm(ida_kernwin.PluginForm):
    """
    Dialog for selecting specific variables/arguments to analyze.

    Provides two selection groups:
    - Function Arguments
    - Local Variables

    Each group has a "Select All" checkbox. Docks to the Output window
    on the right side of the IDA interface.
    """
...
class ReCopilotSettingsDialog(QDialog):
    ...
    1. Model Settings: model name, base URL, API key, prompt template, max output tokens
    2. Analysis Settings: caller/callee depth, context functions, data flow, confirmation, etc.

    Includes validation for required fields and saves settings via settings_manager.
    """
    ...
    def initUI(self):
        ...
        self.model_name_edit = QLineEdit(self.settings_manager.settings['model_name'])
        model_layout.addWidget(self.model_name_edit, 0, 1)

        # Base URL
        model_layout.addWidget(QLabel('Base URL:'), 1, 0)
        self.base_url_edit = QLineEdit(self.settings_manager.settings['base_url'])
        model_layout.addWidget(self.base_url_edit, 1, 1)

        # API Key
        model_layout.addWidget(QLabel('API Key:'), 2, 0)
        self.api_key_edit = QLineEdit(self.settings_manager.settings['api_key'])
        ...
        self.prompt_template_combo.addItems(list(self.PROMPT_TEMPLATE.keys()))
        self.prompt_template_combo.setCurrentText(self.settings_manager.settings['prompt_template'])
        ...
        self.max_output_tokens_spin.setValue(self.settings_manager.settings['max_output_tokens'])
        ...
        self.caller_depth_spin.setValue(self.settings_manager.settings['max_trace_caller_depth'])
        ...
        self.callee_depth_spin.setValue(self.settings_manager.settings['max_trace_callee_depth'])
        ...
        self.context_func_spin.setValue(self.settings_manager.settings['max_context_func_num'])
        ...
        self.data_flow_switch.setChecked(self.settings_manager.settings['data_flow_analysis'])
        ...
        self.measure_info_score_switch.setChecked(self.settings_manager.settings['measure_info_score'])
        ...
        self.need_confirm_switch.setChecked(self.settings_manager.settings['need_confirm'])
        ...
        self.debug_mode_switch.setChecked(self.settings_manager.settings['debug_mode'])
        ...
        self.feedback_switch.setChecked(self.settings_manager.settings['feedback'])
